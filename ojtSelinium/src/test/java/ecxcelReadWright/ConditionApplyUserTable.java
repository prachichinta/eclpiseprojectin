...
public class ConditionApplyUserTable {
		...
		public void printData(){
			...
			if(count==1){
				...
				for(WebElement element : headerRow){
					ExcelReadWithPoi.writeDataInCell("logindata.xlsx", "sheet3", count-1, headerRow.indexOf(element), element.getText());	
				}
				count++;
			}
			while(count<=5){
				...
				for(WebElement element : Row){
					ExcelReadWithPoi.writeDataInCell("logindata.xlsx", "sheet3", count-1, Row.indexOf(element), element.getText());	
